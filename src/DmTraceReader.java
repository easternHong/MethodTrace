...
//            dumpCallTimes();
     ...
     */
    @Override
    public ArrayList<Record> getThreadTimeRecords() {
        Record record;
        ArrayList<Record> timeRecs;
        timeRecs = new ArrayList<Record>();
        ...
        for (ThreadData threadData : mSortedThreads) {
            if (!threadData.isEmpty() && threadData.getId() != 0) {
                record = new Record(threadData, threadData.getRootCall());
                timeRecs.add(record);
            }
        }

        for (Call call : mCallList) {
            record = new Record(call.getThreadData(), call);
            timeRecs.add(record);
        }
    ...
    private void dumpTimeRecs(ArrayList<Record> timeRecs) {
        System.out.print("\nTime Records\n");
        System.out.print("id  t-start    t-end  g-start    g-end  method\n");
        for (Record record : timeRecs) {
